export function verificationPairs(authentication) {
  ...
  const pairs = Object.entries(authentication).map(([name, pair]) => {
    if (!['node', 'payload', 'range'].includes(name) || pair?.request?.profile !== 'malt.authentication/3' ||
        pair?.result?.profile !== 'malt.authentication/3') throw new Error('unsupported authentication evidence')
