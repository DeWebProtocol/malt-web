...
This page describes the released MALT runtime boundary. It is implementation
...
[`v0.0.5`](https://github.com/DeWebProtocol/malt/releases/tag/v0.0.5). It
publishes the client/gateway/core package split, `execution.Executor`,
operation-specific resolve/read profiles, reference-executor naming, and the
`model/unixfs` + `sdk/unixfs` + `runtime/unixfs` split below. Payload selection
is an explicit `@payload` resolve segment; the v0.0.4 v0alpha2 Artifact profile
remains frozen compatibility behavior. The browser verifier's
[published provenance](/verifier/PROVENANCE.json) records the exact full MALT
commit and Go toolchain used to build the deployed WASM.
...
The separate `execution.Executor` performs untrusted `Resolve`, `Read`, and
`Apply` work.
...
## Public CLI and Local Verifier

The public `malt` CLI is intentionally small:
...
content, and exposes the profiled resolve/read endpoints. Its local WASM trust
  boundary tracks MALT v0.0.5: the App verifies both ProofList evidence and the
...
These package names should not redefine the research abstraction. They are
...
| `execution` | Untrusted `Resolve`/`Read`/`Apply` executor over injected semantic/runtime ports. |
